...
$(document).ready(() => {
  ...
  let calculation = () => {
    const cartIcon = $("#itemAmt").text(
      basket.map((x) => x.qty).reduce((x, y) => x + y, 0)
    );
  };
  calculation();
  ...
  let generateCartItems = () => {
    if (basket.length !== 0) {
      return $(".itemCount").html(
        ...
        <div class="itm-card">
          ...
          <div class="right-info">
            ...
            <p id="qty">x${calculation()}</p>
            <p id="pdt-price">Rs. ${search.price}</p>
          </div>
        </div>
      </div>
    </div>`;
      ...
      $("#checkout").text(`
        <h3 class=goback>Go back to Home</h3>
        <a href=./index2.html ><button id=home>Go Back</button></a>`);
    }
  };
  generateCartItems();
});
